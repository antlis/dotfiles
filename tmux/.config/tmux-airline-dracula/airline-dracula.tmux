...
main() {
  ...
  # tmux set-option -g status-right "#S ${left_sep}#[bg=${black},reverse] %Y-%m-%d %H:%M "
  tmux set-option -g status-right "#[fg=${yellow}] #[fg=${white}]#(cd #{pane_current_path}; git rev-parse --abbrev-ref HEAD) #[fg=${light_purple}]#S ${left_sep}#[bg=${black},reverse] %Y-%m-%d %H:%M "
