...
in
{
  ...
  # Corp access via Moscow gateway: weasel-msk-2 runs the corp OpenVPN, exposing
  # internal subnets. corp-msk-route adds a /32 host route for weasel-msk-2 so it
  ...
  # Browser uses ~/.config/umbrella-proxy.pac → routes *.umbrella.moscow through it.
  # Git SSH uses ProxyJump weasel-msk-2 (in ~/.ssh/config).
  ...
  systemd.services.naive-vpn = let
    serverIp = vpnServerIp; # naive exit = weasel-nl origin (www.antlis.xyz -> here); pin out of the tun
